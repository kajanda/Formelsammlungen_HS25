...
db = 609  # Maximaler absoluter Fehler in b  = 3080*0.05+4070*0.05+5030*0.05 (rel. Fehler in b = 5%)
"""==============================================="""

print("Betrachte gestörtes Gleichungssystem Ax~ = b~, wobei b~ in jeder Komponente um maximal " + str(
    db) + " von b abweicht.")
...
dxrel = condA * (db / nib)
print("")
print("Es gilt für den absoluten Fehler von x: ‖x - x~‖∞ ≤ ‖A⁻¹‖∞ * ‖b~ - b‖∞ = " + str(niAinv) + " * " + str(
    db) + " = " + str(dxabs))
print("Es gilt für den relativen Fehler von x: ‖x - x~‖∞ / ‖x‖∞ ≤ cond(A) * (‖b~ - b‖∞ / ‖b‖∞) = " + str(
    condA) + " * (" + str(db) + " / " + str(nib) + ") = " + str(dxrel))
