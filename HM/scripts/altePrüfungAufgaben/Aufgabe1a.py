...
# F = sy.ln(sy.sqrt(x) + 2)  # Linke Seite der Fixpunktgleichung F(x) = x
F = ((sy.exp(x) + sy.exp(-x)) / 2) -(3/2)  # Linke Seite der Fixpunktgleichung F(x) = x
x0 = 1.5  # Startwert der Iteration. (Setze x0 = 1 für divergierenden Fixpunkt in [0, 1])
precision = 1e-6  # Wie genau die Lösung sein soll. Bei 10^-6 = 1e-6
min_iterations = 100  # Minimale Anzahl Iterationen bei divergierenden Folgen, bevor abgebrochen wird.
...
show_plots = True
ap = -1
bp = 2.5  # Intervall, über welchem geplottet werden soll.
"""==============================================="""
...
def check_banach(F, x, a, b, show_plots):
    ...
    print(
        "3. Bedingung (Prüfung optional, da schwierig zu zeigen): F(x) ist Lipschitz-stetig, d.h. |F(x) - F(y)| <= α * |x - y| ∀ x, y ∈ [" + str(
            a) + ", " + str(b) + "]\n")

    steps = 100
    d = (Decimal(b) - Decimal(a)) / steps
    print(
        "Prüfen der ersten Bedingung: Finde maximalen und minimalen Wert von F(x) im Intervall [" + str(a) + ", " + str(
            b) + "], mit Python, indem " + str(steps + 1) + " äquidistante Werte im Intervall berechnet werden.\n")
    print(
        "Für die Bestimmung der x-Werte, an welchem F(x) maximal / minimal wird, kann i.d.R. mit der Monotonie der Funktion agrumentiert werden (\"F(x) ist monoton steigend --> F(x) ist maximal an rechter Intervallgrenze\")")
    ...
    for i in range(steps + 1):
        ...
        yvalue = N(F.subs(x, xvalue)) # print(F.subs(x, xvalue)) # zeige Rechnung vor Berechnung
    ...
    if ymin >= Decimal(a) and ymax <= Decimal(b):
        print("==> Da [" + str(ymin) + ", " + str(ymax) + "] ⊂ [" + str(a) + ", " + str(
            b) + "] ist die erste Bedingung erfüllt.\n")
    else:
        print("==> Da [" + str(ymin) + ", " + str(ymax) + "] ⊄ [" + str(a) + ", " + str(
            b) + "] ist die erste Bedingung NICHT erfüllt.\n")
        return 0
    ...
    print("2. Bedingung: Es existiert ein α = max(|F'(x0)|), mit " + str(a) + " <= x0 <= " + str(
        b) + " so dass 0 < α < 1")
    dF = sy.diff(F, x)
    print("F'(x) = " + str(dF))
    ...
    print("Maximaler Wert von F'(x0) in [" + str(a) + ", " + str(b) + "]: F'(" + str(xmax) + ") = α = " + str(
        alpha.evalf()))
    print(
        "Für die Bestimmung des x-Werts, an welchem F'(x) maximal wird, kann i.d.R. mit der Monotonie der Funktion agrumentiert werden (\"F'(x) ist monoton steigend --> F'(x) ist maximal an rechter Intervallgrenze\")")
    ...
    print(
        "\nPrüfen der dritten Bedingung (optional):  Lipschitz-Stetigkeit |F(x) - F(y)| <= α * |x - y| ∀ x, y ∈ [" + str(
            a) + ", " + str(b) + "], ")
    print(
        "entweder anschaulich durch Plot, oder numerisch grobe Prüfung mit Python (wird durch dieses Skript durchgeführt).\n")
    ...
    for i in range(steps + 1):
        ...
        for j in range(steps + 1):
            ...
            if dy > alpha * dx:
                print("==> Lipschitz-Stetigkeit ist NICHT erfüllt für x = " + str(x1value) + " und y = " + str(
                    x2value) + "! |F(x) - F(y)| = " + str(dy) + ", α * |x - y| = " + str(alpha * dx))
                lipschitzOK = False
                return 0
...
if check_banach_and_calculate_estimations:
    ...
    if alpha > 0:
        ...
        n_apriori = (math.log((max_a_priori_error * (1 - alpha)) / (abs(F.subs(x, x0) - x0)))) / (math.log(alpha))
        print("=> Aus A-Priori-Abschätzung folgt, dass absoluter Fehler < " + str(
            max_a_priori_error) + " ist, für alle n ≥ " + str(n_apriori) + ".\n\n")
...
while abs(xn[n] - xn[n - 1]) > precision:
    ...
    if check_banach_and_calculate_estimations and alpha > 0:
        ...
        print("n = " + str(n) + ": x" + str(n) + " = F(x" + str(n - 1) + ") = " + str(xn[n]) + ", Δ = " + str(
            abs(xn[n] - xn[n - 1])) + ", Δ_a_posteriori ≤ " + str(dx_a_posteriori.evalf()))
    else:
        print("n = " + str(n) + ": x" + str(n) + " = F(x" + str(n - 1) + ") = " + str(xn[n]) + ", Δ = " + str(
            abs(xn[n] - xn[n - 1])))
...
# 1 a)
# Die Lösung ist x_18=-0.413348076004772
# Man kriegt nicht die von x_0 nächste Lösung, da der Fixpunkt der nahe x_0 liegt in F(x) eine Steigung >1 hat und somit divergiert.
