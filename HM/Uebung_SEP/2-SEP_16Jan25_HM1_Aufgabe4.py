...
show_plots = True
ap = -2;
bp = 4  # Intervall, über welchem geplottet werden soll.
"""==============================================="""

Fl = sy.lambdify(x,F) # wird fürs nummerisch rechnen verwendet
...
while abs(xn[n] - xn[n - 1]) > precision:
    ...
    if check_banach_and_calculate_estimations and alpha > 0:
        ...
        print("n = " + str(n) + ": x" + str(n) + " = F(x" + str(n - 1) + ") = " + str(xn[n]) + ", Δ = " + str(
            abs(xn[n] - xn[n - 1])) + ", Δ_a_posteriori ≤ " + str(dx_a_posteriori))
    else:
        print("n = " + str(n) + ": x" + str(n) + " = F(x" + str(n - 1) + ") = " + str(xn[n]) + ", Δ = " + str(
            abs(xn[n] - xn[n - 1])))
print('Anzahl Schritte: ',n)
...
"""==================== INPUT ===================="""
f = ((sy.exp(x) + sy.exp(-x)) / 2) - (3/2) - x  # Achtung es ist ein Nullstellenproblem f(x)=0!!!!!!!!
x0 = 1.4
x1 = 1.6
max_error = 1e-6
"""==============================================="""
